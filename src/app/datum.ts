...
export class DatumService extends BasicService {
  ...
  queryAll:string=`${this.queryUrl}?
                    $query=select date_extract_y(datum_eerste_toelating_dt),
                    count(*) where datum_eerste_toelating_dt is not null 
                    group by date_extract_y_datum_eerste_toelating_dt
                    order by date_extract_y_datum_eerste_toelating_dt`;
